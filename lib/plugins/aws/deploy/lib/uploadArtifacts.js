  ...
  uploadFunctionsAndLayers() {
    ...
    return BbPromise.map(
      ...
      artifactFilePath => {
        ...
        const [size, units] = filesize(stats.size).split(' ');
        this.serverless.cli.log(
          `Uploading service ${fileName} file to S3 (${filesize(stats.size)})...`
        );
        if (units === 'GB' || (units === 'MB' && size > 300)) {
          this.serverless.cli.log(
            `Warning: service ${fileName} may be approaching the lambda limit for file size (${filesize(stats.size)})...`
          );
        }
